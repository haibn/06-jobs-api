...
const getAllSpreadsheets = async (req, res) => {
    const spreadsheets = await Spreadsheet.find({createdBy: req.user.userId}).sort('createdAt')
...
const getSpreadsheet = async (req, res) => {
    ...
    if (!spreadsheet) {
        throw new NotFoundError(`The spreadsheet with the id ${spreadsheetId} does not exists`)
    }
